...
import { MesaElectoral, MesaElectoralFormData, Recinto } from '../types';
...
const MesasElectoralesPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if mesa number already exists in the same recinto
    ...
    try {
      if (editingMesa) {
        await apiService.updateMesaElectoral(editingMesa.id, formData);
        toast.success('Mesa electoral actualizada exitosamente');
      } else {
        await apiService.createMesaElectoral(formData);
  ...
  const handleDelete = async (mesa: MesaElectoral) => {
    ...
    try {
      await apiService.deleteMesaElectoral(mesa.id);
