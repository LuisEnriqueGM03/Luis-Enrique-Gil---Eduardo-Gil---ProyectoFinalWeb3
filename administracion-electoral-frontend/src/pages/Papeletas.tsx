...
import { PapeletaDisponible, Papeleta } from '../models/papeleta';
import { papeletasService } from '../services/papeletasService';
...
const PapeletasPage: React.FC = () => {
  ...
  const fetchPapeletasDisponibles = async () => {
    try {
      setLoading(true);
      const response = await papeletasService.getPapeletasDisponibles();
  ...
  const handleVerPapeleta = async (papeleta: PapeletaDisponible) => {
    try {
      const response = await papeletasService.getPapeletaPorSeccion(papeleta.seccion_id);
