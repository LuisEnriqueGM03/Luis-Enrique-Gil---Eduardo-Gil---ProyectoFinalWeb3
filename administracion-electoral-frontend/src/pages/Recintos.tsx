...
const RecintosPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRecinto) {
        await recintosService.updateRecinto(editingRecinto.id, formData);
        toast.success('Recinto actualizado exitosamente');
      } else {
        await recintosService.createRecinto(formData);
  ...
  const handleDelete = async (recinto: Recinto) => {
    ...
    try {
      await recintosService.deleteRecinto(recinto.id);
                      ...
                      <div className="mt-1 space-y-1">
                        ...
                        {recinto.latitud && recinto.longitud && (
                          <p className="text-xs text-secondary-500">
                            Ubicación: {Number(recinto.latitud).toFixed(6)}, {Number(recinto.longitud).toFixed(6)}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[80vh]">
              ...
              <div>
                ...
                <LocationSelector
                  center={selectedLocation || [-17.783, -63.182]}
                  zoom={12}
