...
import { MesaElectoral } from '../models/mesaElectoral';
...
const JuradosPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingJurado) {
        await juradosService.updateJurado(editingJurado.id, formData);
        toast.success('Jurado actualizado exitosamente');
      } else {
        await juradosService.createJurado(formData);
  ...
  const handleDelete = async (jurado: Jurado) => {
    ...
    try {
      await juradosService.deleteJurado(jurado.id);
