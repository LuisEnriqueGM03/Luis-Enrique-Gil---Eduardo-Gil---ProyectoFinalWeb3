...
import { Candidatura, CandidaturaFormData, Cargo } from '../types';
...
const CandidaturasPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if sigla already exists for the same cargo
    ...
    try {
      ...
      if (editingCandidatura) {
        await apiService.updateCandidatura(editingCandidatura.id, submitData);
        toast.success('Candidatura actualizada exitosamente');
      } else {
        await apiService.createCandidatura(submitData);
  ...
  const handleDelete = async (candidatura: Candidatura) => {
    ...
    try {
      await apiService.deleteCandidatura(candidatura.id);
